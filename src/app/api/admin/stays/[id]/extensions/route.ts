...
import { applyDiscount, calculateOfferAmount, derivePaymentStatus } from "@/lib/pricing";
...
export const POST = withErrorHandler(async (req: Request, { params }: { params: Promise<{ id: string }> }) => {
  ...
  if (!["planifie", "en_cours"].includes(stay.status)) {
    throw new ApiError(409, "Seuls les séjours planifiés ou en cours peuvent être prolongés.");
  }
